...
// Force country + state on checkout update (billing_country field is hidden)
...
// Force country + state in shipping packages
...
function jimee_sync_billing_to_shipping_state( $packages ) {
    ...
    foreach ( $packages as &$pkg ) {
        $pkg['destination']['country'] = 'DZ';
        if ( ! empty( $state ) ) {
            $pkg['destination']['state'] = $state;
        }
    }
    return $packages;
}
