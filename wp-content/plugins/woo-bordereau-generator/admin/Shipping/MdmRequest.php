<?php
/**
 * MDM Express API Request Handler.
 * 
 * This class manages HTTP requests to the MDM Express API,
...
/**
 * MDM Express API request handler class.
 * 
 * Handles all HTTP communication with the MDM Express API,
...
class MdmRequest
{
    ...
    public function post($url, $data, string $method = self::POST)
    {
        ...
        // Set the authentication header with API key
    ...
    public function fetchRequest($url, $data = [])
    {
        try {
            ...
            if (isset($response['error'])) {
                ...
                new WP_Error($response['error']['code'] ?? 400, $response['error']['message']);
            }
    ...
    /**
     * Get the authentication header with API key.
     *
    ...
    {
        return get_option( $this->provider['slug'] . '_api_key' ) ?: '';
    }
