...
class EcoTrackShipping extends BordereauProvider implements BordereauProviderInterface
{
	...
	public function generate()
	{
		...
		if (isset($this->data['is_fragile']) && $this->data['is_fragile'] == "true") {
			$data['fragile'] = 1;
		}

		// Add weight if provided (ecotrack surpoid)
		$weight = 0;
		if (isset($this->data['ecotrack_poids']) && $this->data['ecotrack_poids'] > 0) {
			$weight = (float) $this->data['ecotrack_poids'];
		} else {
			// Calculate weight from order products
			$weight = $this->calculateOrderWeight($params);
		}

		// Add weight if surpoid is enabled and weight is greater than 0
		$overweightEnabled = get_option('recalculate_' . $this->provider['slug'] . '_overweight', 'recalculate-without-overweight');

		if ($overweightEnabled === 'recalculate-with-overweight' && $weight > 0) {
			$data['weight'] = $weight;
		}
		...
		if (isset($this->data['product_exchanged']) && $this->data['operation_type'] == 2) {
		...
		if (get_option($this->provider['slug'].'_avec_stock') === 'with-stock') {
			...
			foreach ($params['line_items'] as $k => $p) {
				$qty[$k] = $p['quantity'];
				$product = $p->get_product();
				if ($product) {
					$skus[$k] = $product->get_sku();
				}
			}

			$data['quantite'] = implode(",", $qty);
			$data['produit'] = implode(",", $skus);
		}


		// Use the EcoTrackRequest class for API requests with form-encoded data
		$response = $this->request->post($url, $data, true);

		// Check for rate limit error
		if (isset($response['error']) && $response['error'] === true) {
			$logger = wc_get_logger();
			$logger->error('EcoTrack API Error: ' . ($response['message'] ?? 'Unknown error'), array('source' => 'ecotrack-api'));

			wp_send_json([
				'error' => true,
				'message' => $response['message'] ?? "Rate limit exceeded. Please try again later."
	...
	{
		$url = $this->provider['api_url']. "/get/tracking/info?tracking={$tracking_number}";
	 ...
	 * @return array
	 */
	public function get_communes(int $wilaya_id, bool $hasStopDesk): array
	{
		...
		if (! file_exists($path)) {
			...
			if (isset($response['error']) && $response['error'] === true) {
				\wp_send_json([
					'error' => true,
					'message' => $response['message'] ?? \__("Rate limit exceeded. Please try again later.", 'woo-bordereau-generator')
				], 429); // Use 429 status code for rate limit errors
			}
	...
	{
		if ($this->api_token) {
			$url = $this->provider['api_url'] . "/get/orders?tracking=" . $tracking_number;
	...
	{
		$url = $this->provider['api_url'] . "/delete/order?tracking=" . $tracking_number;

		// Use the EcoTrackRequest class for API requests with DELETE method
	 ...
	 * @since 4.8.0
	 */
	private function calculateOrderWeight(array $params): float
	{
		$total_weight = 0;
			...
			foreach ($params['line_items'] as $item) {
				...
				if ($product_id) {
					...
					if ($product && $product->get_weight()) {
						...
						$total_weight += (float) $product->get_weight() * (int) $quantity;
					}
				}
			}
		}
